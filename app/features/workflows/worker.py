import asyncio
import sys
import uuid
from datetime import UTC, datetime, timedelta

from langgraph.errors import GraphInterrupt
from langgraph.types import Command
from loguru import logger
from psycopg_pool import AsyncConnectionPool
from sqlalchemy import select, update

from app.core.config.settings import settings
from app.core.db.base import async_session
from app.features.workflows.master_graph import (
    PrometheusMetricsCallbackHandler,
    get_master_graph,
)
from app.features.workflows.models import AgentTask


class AgentWorker:
    def __init__(self, name: str = "default-worker"):
        self.name = name
        self.running = False
        self.pool = None

    async def start(self):
        logger.info(f"Starting agent worker: {self.name}")
        self.running = True

        # Initialize connection pool once (Issue #3)
        self.pool = AsyncConnectionPool(
            conninfo=settings.postgres_url.replace("postgresql+psycopg", "postgresql"),
            max_size=10,
        )
        await self.pool.wait()

        # Periodic stale task cleanup
        reaper_task = asyncio.create_task(self.reap_stale_tasks_loop())

        try:
            while self.running:
                try:
                    task_processed = await self.poll_and_execute()
                    if not task_processed:
                        await asyncio.sleep(1.0)
                except Exception as e:
                    logger.exception(f"Error in worker main loop: {e}")
                    await asyncio.sleep(2.0)
        finally:
            reaper_task.cancel()
            await self.stop()

    async def stop(self):
        logger.info(f"Stopping agent worker: {self.name}")
        self.running = False
        if self.pool:
            await self.pool.close()
            logger.info("Closed worker database connection pool.")

    async def poll_and_execute(self) -> bool:
        """Poll the DB for a pending task and execute it."""
        async with async_session() as session:
            stmt = (
                select(AgentTask)
                .where(AgentTask.status == "pending")
                .order_by(AgentTask.created_at)
                .limit(1)
                .with_for_update(skip_locked=True)
            )
            res = await session.execute(stmt)
            task = res.scalar_one_or_none()

            if not task:
                return False

            logger.info(f"Claimed task: {task.id} (Graph: {task.graph_name})")

            # Check attempts bounds BEFORE claiming and starting (Issue #5)
            if task.attempts >= 3:
                logger.error(
                    f"Task {task.id} exceeded max attempts ({task.attempts}). Marking failed."
                )
                task.status = "failed"
                task.error = (
                    f"Max execution attempts exceeded (attempts: {task.attempts})"
                )
                task.completed_at = datetime.now(UTC)
                await session.commit()
                return True

            task.status = "running"
            task.locked_at = datetime.now(UTC)
            task.locked_by = self.name
            task.attempts += 1
            await session.commit()

            task_id = task.id
            payload = task.payload
            thread_id = task.thread_id
            graph_name = task.graph_name

        try:
            if graph_name != "main_graph":
                raise ValueError(f"Unsupported graph: {graph_name}")

            # Pass the connection pool to reuse connection (Issue #3)
            master_graph = await get_master_graph(self.pool)

            # Non-intrusive metrics callback (Issue #1)
            metrics_handler = PrometheusMetricsCallbackHandler()
            config = {
                "configurable": {"thread_id": thread_id},
                "metadata": {
                    "thread_id": thread_id,
                    "stage": "initial",
                },
                "tags": [f"worker:{self.name}", f"task:{task_id}"],
                "callbacks": [metrics_handler],
            }

            # Extract initial state from payload
            initial_state = payload.get("initial_state") or payload
            resume_data = payload.get("resume_data")

            logger.info(f"Running master graph for thread: {thread_id}")

            try:
                # Run the graph
                clear_resume_data = False
                if resume_data is not None:
                    state_before = await master_graph.aget_state(config)
                    if state_before and state_before.values:
                        config["metadata"]["stage"] = state_before.values.get(
                            "stage", "unknown"
                        )

                    await master_graph.ainvoke(Command(resume=resume_data), config)
                    clear_resume_data = True
                else:
                    await master_graph.ainvoke(initial_state, config)

                state = await master_graph.aget_state(config)
                if state.next:
                    # Thread paused at interrupt
                    await self.update_task_status(
                        task_id,
                        status="paused",
                        result={"next_nodes": list(state.next)},
                        clear_resume_data=clear_resume_data,
                    )
                else:
                    # Graph completed
                    await self.update_task_status(
                        task_id,
                        status="completed",
                        result=state.values,
                        clear_resume_data=clear_resume_data,
                    )
            except GraphInterrupt:
                # Graph hit an interrupt HITL
                state = await master_graph.aget_state(config)
                await self.update_task_status(
                    task_id,
                    status="paused",
                    result={"next_nodes": list(state.next)},
                    clear_resume_data=clear_resume_data,
                )

        except Exception as e:
            logger.exception(f"Task {task_id} failed with error: {e}")
            await self.update_task_status(task_id, status="failed", error=str(e))

        return True

    async def update_task_status(
        self,
        task_id: uuid.UUID,
        status: str,
        result: dict | None = None,
        error: str | None = None,
        clear_resume_data: bool = False,
    ):
        """Update the final task state in the DB in a single transaction (Issue #8)."""
        async with async_session.begin() as session:
            task = await session.get(AgentTask, task_id)
            if task:
                task.status = status
                task.updated_at = datetime.now(UTC)
                if status in ("completed", "failed", "paused"):
                    task.completed_at = datetime.now(UTC)
                if result is not None:
                    task.result = self.serialize_clean(result)
                if error is not None:
                    task.error = error
                if clear_resume_data and "resume_data" in task.payload:
                    new_payload = task.payload.copy()
                    new_payload.pop("resume_data", None)
                    task.payload = new_payload
                task.locked_by = None
                task.locked_at = None

    def serialize_clean(self, obj):
        if isinstance(obj, dict):
            return {k: self.serialize_clean(v) for k, v in obj.items()}
        elif isinstance(obj, list):
            return [self.serialize_clean(x) for x in obj]
        elif isinstance(obj, (uuid.UUID | datetime)):
            return str(obj)
        elif hasattr(obj, "content"):  # LangChain messages
            return {"type": obj.__class__.__name__, "content": str(obj.content)}
        return obj

    async def reap_stale_tasks_loop(self):
        while self.running:
            try:
                await self.reap_stale_tasks()
            except Exception as e:
                logger.error(f"Error in stale task reaper: {e}")
            await asyncio.sleep(300.0)

    async def reap_stale_tasks(self):
        ten_mins_ago = datetime.now(UTC) - timedelta(minutes=10)
        async with async_session.begin() as session:
            stmt = (
                update(AgentTask)
                .where(
                    AgentTask.status == "running", AgentTask.locked_at < ten_mins_ago
                )
                .values(status="pending", locked_by=None, locked_at=None)
                .returning(AgentTask.id)
            )
            res = await session.execute(stmt)
            stale_ids = res.scalars().all()
            for tid in stale_ids:
                logger.warning(f"Reclaiming stale task: {tid}")


def main():
    worker_name = f"worker-{uuid.uuid4().hex[:6]}"
    if len(sys.argv) > 2 and sys.argv[1] == "--name":
        worker_name = sys.argv[2]

    worker = AgentWorker(worker_name)
    try:
        asyncio.run(worker.start())
    except KeyboardInterrupt:
        logger.info("Worker stopped by user.")
    except Exception as e:
        logger.critical(f"Unhandled worker error: {e}")


if __name__ == "__main__":
    main()
